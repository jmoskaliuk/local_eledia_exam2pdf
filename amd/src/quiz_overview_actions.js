 ...
 */
const insertAfter = (row, index, cell) => {
 ...
 * @return {number}
 */
const findCellIndexForLogicalColumn = (row, logicalIndex) => {
    const cells = row.children;
    let cursor = 0;
    for (let i = 0; i < cells.length; i++) {
        let span = parseInt(cells[i].getAttribute('colspan') || '1', 10);
        if (!span || span < 1) {
            span = 1;
        }
 ...
 * actions column. The grades table ships with `align-top` utilities on
 * some cells, which would leave the inline action buttons floating at
 * the top.
 *
 * @param {HTMLElement} row
 */
const normaliseRowCellAlignment = (row) => {
    const cells = row.children;
    for (let i = 0; i < cells.length; i++) {
    ...
    const questions = row.querySelectorAll('td span.que');
    for (let qi = 0; qi < questions.length; qi++) {
        const qcell = questions[qi].closest('td');
        if (qcell) {
            qcell.style.textAlign = 'center';
            qcell.style.setProperty('vertical-align', 'middle', 'important');
        }

        const qlink = questions[qi].closest('a');
        if (qlink) {
    ...
    const qicons = row.querySelectorAll('td span.que img.icon, td span.que i.icon, td span.que .questionflag');
    for (let ii = 0; ii < qicons.length; ii++) {
 ...
 */
const expandRowAtGradePosition = (row, gradeIndex) => {
    const anchorIdx = findCellIndexForLogicalColumn(row, gradeIndex);
    if (anchorIdx < 0) {
        return false;
    }

    const anchorCell = row.children[anchorIdx];
    let span = parseInt(anchorCell.getAttribute('colspan') || '1', 10);
    if (!span || span < 1) {
        span = 1;
    }
...
/**
 * Best-effort extraction of the attempt ID from an overview row. Tries
 * the hidden `attemptid[]` checkbox first, then the "Review attempt"
 * link's `attempt=` query parameter.
 *
 * @param {HTMLElement} row
 * @return {number} attempt ID, or 0 if not found.
 */
const extractAttemptId = (row) => {
    const checkbox = row.querySelector('input[type="checkbox"][name="attemptid[]"]');
    ...
    const links = row.querySelectorAll('a[href*="/mod/quiz/review.php?"]');
    for (let i = 0; i < links.length; i++) {
        const href = links[i].getAttribute('href') || '';
        const match = href.match(/[?&]attempt=(\d+)/);
        if (match && match[1]) {
...
/**
 * Locate the quiz overview report table. mod_quiz's overview report
 * renders a table with the `generaltable` + `grades` class combination.
 *
 * @return {HTMLElement|null}
 */
const findOverviewTable = () => document.querySelector('table.generaltable.grades');

/**
 * Inject a single <style> tag into the document head with the CSS that
 * guarantees consistent vertical centring of every cell (including the
 * grade column and per-question score columns) even after mod_quiz's
 * report enhancements redraw the table. The `.icon` margin reset makes
 * icon-only buttons render with the icon truly centred rather than
 * shifted left by Moodle-core's default `margin-right: .5rem`.
 *
 * Safe to call multiple times — the `data-local-eledia-exam2pdf`
 * attribute marks the tag so it is only added once.
 */
const injectAlignmentStyles = () => {
    const marker = 'data-local-eledia-exam2pdf-actions-style';
...
/**
 * Initialise the module. Safe to call multiple times — the header check
 * ensures the column is only added once per table.
 *
 * @param {Object} args
...
export const init = (args) => {
    ...
    // Always inject the style rules — even if the column has already been
    ...
    const gradeLabel = args.gradeLabel;
    const headers = Array.prototype.slice.call(headrow.children);
    let gradeIndex = -1;
    for (let i = 0; i < headers.length; i++) {
        const htext = (headers[i].textContent || '').trim().toLowerCase();
        if (!htext) {
            continue;
        }
    ...
    const btnBaseClass = 'btn btn-sm d-inline-flex align-items-center justify-content-center';
     ...
     * @return {string}
     */
    const buildActionsHtml = (attemptid) => {
        if (!attemptid) {
            return '-';
        }

        const downloadUrl = args.downloadBaseUrl.replace(/([?&]attemptid=)0(?!\d)/, '$1' + attemptid);
        let html = '<a href="' + downloadUrl + '" class="' + btnBaseClass + ' btn-outline-primary"'
            + ' aria-label="' + args.downloadLabel + '" title="' + args.downloadLabel + '">'
            + args.downloadIcon + '</a>';

        if (args.canRegenerate) {
            const regenerateUrl = args.regenerateBaseUrl.replace(/([?&]attemptid=)0(?!\d)/, '$1' + attemptid);
    ...
    for (let fi = 0; fi < footrows.length; fi++) {
        const row = footrows[fi];
        normaliseRowCellAlignment(row);
        if (row.querySelector('.local-eledia-exam2pdf-cell-actions')) {
            continue;
        }
    ...
    window.setTimeout(() => {
        const rerows = table.querySelectorAll('tbody tr, tfoot tr');
        for (let ri = 0; ri < rerows.length; ri++) {
